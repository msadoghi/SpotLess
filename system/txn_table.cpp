...
void TxnTable::free()
{
    for (uint32_t i = 0; i < pool_size; i++)
    {
        if(pool[i] && pool[i]->head){
            txn_node_t t_node = pool[i]->head;
            // Check if matching txn manager found in the global pool.
            while (t_node != NULL)
            {
                int txn_id = t_node->txn_man->get_txn_id();
                txn_node_t txn_next = t_node->next;
    ...
    txn_man_pool.release();
    txn_table_pool.release();
}
...
void TxnTable::release_transaction_manager(uint64_t thd_id, uint64_t txn_id, uint64_t batch_id)
{
    ...
    while (t_node != NULL)
    {
        if (is_matching_txn_node(t_node, txn_id, batch_id))
        {
            ...
            LIST_REMOVE_HT(t_node, pool[pool_id]->head, pool[pool_id]->tail);
            --pool[pool_id]->cnt;
            break;
        }
        t_node = t_node->next;
