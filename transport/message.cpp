...
#if SEPARATE
	case HOTSTUFF_PROPOSAL_MSG:{
		HOTSTUFFProposalMsg *m_msg = (HOTSTUFFProposalMsg *)msg;
		m_msg->release();
		delete m_msg;
		break;
	}
...
#if USE_CRYPTO
	#if MAC_SYNC
		string message2 = getString(g_node_id);	// MAC
...
#if MAC_SYNC
	string message2 = getString(this->return_node_id);
...
#if THRESHOLD_SIGNATURE && !MAC_SYNC
#if !CHAINED
	assert(highQC.genesis || (highQC.ThresholdSignatureVerify(HOTSTUFF_PREP_MSG)));
#else
	QuorumCertificate pQC = get_g_preparedQC(instance_id);
	assert(highQC.genesis || (highQC.viewNumber == pQC.viewNumber && highQC.batch_hash == pQC.batch_hash) || highQC.ThresholdSignatureVerify(HOTSTUFF_NEW_VIEW_MSG));
#endif
#endif
...
void HOTSTUFFNewViewMsg::digital_sign()
{
	unsigned char message[32];
...
void HOTSTUFFNewViewMsg::release(){
	hash.clear();
...
uint64_t HOTSTUFFProposalMsg::get_size()
{
	uint64_t size = Message::mget_size();
	size += sizeof(view);
	size += sizeof(uint64_t) * index.size();
...
void HOTSTUFFProposalMsg::add_request_msg(uint idx, Message * msg){
     if(requestMsg[idx]){
 		Message::release_message(requestMsg[idx]);
     }
 #if BANKING_SMART_CONTRACT
...
// Initialization
void HOTSTUFFProposalMsg::init(uint64_t instance_id)
{
	// Only primary should create this message
	#if !PVP
	assert(get_view_primary(get_current_view(0)) == g_node_id);
...
void HOTSTUFFProposalMsg::copy_from_txn(TxnManager *txn)
{
	// Setting txn_id 2 less than the actual value.
	this->txn_id = txn->get_txn_id() - 2;
	this->batch_size = get_batch_size();
...
#if BANKING_SMART_CONTRACT
void HOTSTUFFProposalMsg::copy_from_txn(TxnManager *txn, BankingSmartContractMessage *clqry)
{
	// Index of the transaction in this bacth.
	uint64_t txnid = txn->get_txn_id();
	uint64_t idx = txnid % get_batch_size();
...
#else
void HOTSTUFFProposalMsg::copy_from_txn(TxnManager *txn, YCSBClientQueryMessage *clqry)
{
	// Index of the transaction in this bacth.
	uint64_t txnid = txn->get_txn_id();
	uint64_t idx = txnid % get_batch_size();
...
string HOTSTUFFProposalMsg::getString(uint64_t sender){
	string message = std::to_string(sender);
	for (uint i = 0; i < get_batch_size(); i++)
	{
		message += std::to_string(index[i]);
...
void HOTSTUFFProposalMsg::sign(uint64_t dest_node)
{
#if USE_CRYPTO
	#if MAC_SYNC
...
void HOTSTUFFProposalMsg::copy_from_buf(char *buf)
{
	Message::mcopy_from_buf(buf);
...
void HOTSTUFFProposalMsg::copy_to_buf(char *buf)
{
	Message::mcopy_to_buf(buf);
...
//makes sure message is valid, returns true for false
bool HOTSTUFFProposalMsg::validate(uint64_t thd_id)
{
...
void HOTSTUFFProposalMsg::release()
{
	index.release();
	for (uint64_t i = 0; i < requestMsg.size(); i++)
	{
...
uint64_t HOTSTUFFGenericMsg::get_size()
{
	uint64_t size = Message::mget_size();
	size += sizeof(view);
	size += sizeof(index);
...
string HOTSTUFFGenericMsg::toString()
{
	string signString = std::to_string(this->view);
	signString += '_' + std::to_string(this->index) + '_' +
				  this->hash + '_' + std::to_string(this->return_node) +
...
void HOTSTUFFGenericMsg::sign(uint64_t dest_node)
{
#if USE_CRYPTO
	#if MAC_SYNC
		string message2 = this->toString();	// MAC
...
#if MAC_VERSION
void HOTSTUFFGenericMsg::digital_sign()
{
	unsigned char message[32];
	string metadata = this->hash + this->highQC.to_string();
	metadata = calculateHash(metadata);
...
void HOTSTUFFGenericMsg::copy_from_txn(TxnManager *txn){
	Message::mcopy_from_txn(txn);
	#if !PVP
	uint64_t instance_id = 0;
	#else
...
void HOTSTUFFGenericMsg::copy_from_buf(char *buf)
{
	Message::mcopy_from_buf(buf);
...
void HOTSTUFFGenericMsg::copy_to_buf(char *buf)
{
	Message::mcopy_to_buf(buf);
...
//makes sure message is valid, returns true or false;
bool HOTSTUFFGenericMsg::validate()
{
#if USE_CRYPTO
...
void HOTSTUFFGenericMsg::release(){
 	hash.clear();
 	highQC.release();
}
